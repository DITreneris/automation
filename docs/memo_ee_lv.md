...
**Note (2026):** Site now has **5 locales** (LT, EN, ET, LV, JA); EN is default. JA is manual; this memo focuses on ET/LV generator workflow. See [MULTILINGUAL_STRUCTURE.md](MULTILINGUAL_STRUCTURE.md) for full routing.
...
2. **Language switcher** — five languages on all pages: **Lietuvių**, **English**, **Eesti**, **Latviešu**, **日本語** (no flags).
3. **Root `/`** — default **`/en/`**; `localStorage` / `navigator` also route to `lt`, `et`, `lv`, `ja` per [MULTILINGUAL_STRUCTURE.md](MULTILINGUAL_STRUCTURE.md).
4. **`hreflang`** — includes `ja`; pathname strips `/(lt|en|et|lv|ja)(/…|$)`.
...
- **Product URL:** Canonical `https://www.promptanatomy.info/en` everywhere; brand name “Prompt Anatomy” often kept in English in UI.
- **Library production:** `https://www.promptanatomy.info/` (Vercel: `https://automation-seven-ochre.vercel.app/`).
